...
const whatsappCloudHandler = require('../channels/whatsapp/whatsappCloudHandler');
const messengerHandler = require('../channels/messenger/messengerHandler');
const logger = require('../utils/logger');

const router = express.Router();

/**
 * WhatsApp Cloud API and Messenger both use ONE shared webhook URL for
 * every tenant on your whole platform (Meta doesn't support per-tenant
 * webhook URLs the way Telegram does) — set these two URLs once in
 * the Meta App Dashboard:
 *   https://api.yourdomain.com/api/webhooks/whatsapp
 *   https://api.yourdomain.com/api/webhooks/messenger
 * Routing to the right tenant happens inside each handler, keyed by
 * the phone_number_id / page_id Meta includes in every payload.
 */
router.get('/whatsapp', async (req, res) => {
  const challenge = await whatsappCloudHandler.verifyWebhook(req.query);
  if (challenge === null) return res.sendStatus(403);
  res.status(200).send(challenge);
});

router.post('/whatsapp', async (req, res) => {
  res.sendStatus(200); // ack immediately — Meta disables the webhook after 5 slow/failed responses
  try {
    await whatsappCloudHandler.handleWebhookEvent(req.body);
  } catch (err) {
    logger.error('whatsapp_cloud_webhook_failed', { error: err.message });
  }
});

router.get('/messenger', async (req, res) => {
  const challenge = await messengerHandler.verifyWebhook(req.query);
  if (challenge === null) return res.sendStatus(403);
  res.status(200).send(challenge);
});
...
router.post('/messenger', async (req, res) => {
  res.sendStatus(200);
  try {
    await messengerHandler.handleWebhookEvent(req.body);
  } catch (err) {
    logger.error('messenger_webhook_failed', { error: err.message });
  }
});
