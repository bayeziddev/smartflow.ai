...
auth.post('/register', async (c) => {
  ...
  const token = await sign({ tenantId: result.insertId, role: 'owner', exp: Math.floor(Date.now() / 1000) + 60 * 60 * 24 * 7 }, c.env.JWT_SECRET, 'HS256');
...
auth.post('/login', async (c) => {
  ...
  const rows = await query(c.env, c.executionCtx, 'SELECT id, password_hash, role FROM users WHERE email = ? AND is_active = 1', [email]);
  const user = rows[0];
  ...
  const token = await sign({ tenantId: user.id, role: user.role, exp: Math.floor(Date.now() / 1000) + 60 * 60 * 24 * 7 }, c.env.JWT_SECRET, 'HS256');
