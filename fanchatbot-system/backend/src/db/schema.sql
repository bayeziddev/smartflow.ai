...
CREATE TABLE IF NOT EXISTS tenant_api_configs (
  ...
  provider           ENUM('openai','gemini','manus','groq') NOT NULL,
...
-- Not explicitly requested but required to back "Channel Management"
-- toggles — kept minimal here, extended in the channel-handler phase.
-- ----------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS channel_configs (
  ...
  external_identifier VARCHAR(191) NULL,          -- phone number / bot username / inbox address
  metadata_json      JSON NULL,                    -- non-secret channel metadata only
  created_at         DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at         DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS ai_usage_logs (
  id                 BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  tenant_id          BIGINT UNSIGNED NOT NULL,
  provider_requested ENUM('openai','gemini','manus','groq') NOT NULL,
  provider_used      ENUM('openai','gemini','manus','groq') NOT NULL,
